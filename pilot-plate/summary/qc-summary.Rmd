...
summarydata$HDAC4 = unlist(c(counts["ENSMUSG00000026313",]))
summarydata$HDAC4 = log(summarydata$HDAC4 + 1)
...
pheatmap(cone_rod_counts, fontsize=6, annotation_row=anno_df,,
         ...
         annotation=summarydata[, c("rescued", "plate", "Mapped", "ds.red.z", "HDAC4")])
ggsave("heatmap-cone-rod-only.pdf")
housekeeping_counts = lcounts[subset(all_types, type == "housekeeping")$id,]
rownames(housekeeping_counts) = all_types[match(rownames(housekeeping_counts), all_types$id), "mgi_symbol"]
pheatmap(housekeeping_counts, fontsize=6, annotation_row=anno_df,
         cluster_rows=FALSE, annotation_colors=ann_colors,
         annotation=summarydata[, c("rescued", "plate", "Mapped", "ds.red.z")])
ggsave("pilot-plate-housekeeping-only-heatmap.pdf")
```
...
housekeeping_counts = lsmartseq[subset(all_types, type == "housekeeping")$id,]
rownames(housekeeping_counts) = all_types[match(rownames(housekeeping_counts), all_types$id), "mgi_symbol"]
pheatmap(housekeeping_counts, fontsize=6, annotation_row=anno_df,
         cluster_rows=FALSE, annotation_colors=ann_colors)
ggsave("smartseq-housekeeping-only-heatmap.pdf")
```
