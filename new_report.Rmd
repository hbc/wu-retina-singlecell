...
```{r gene-heatmap}
pheatmap(sqrt(sqrt(counts)), annotation_colors=list(class=class_colors), annotation=class_df,
    show_rownames=FALSE, show_colnames=FALSE, cluster_rows=FALSE)
```
...
roska_cone = c("Pde6c","Opn1sw","Fabp7","Gnat2","Opn1mw","Pde6h","Clca3",
              ...
              "Rbp3","Lcn2","Acbd6")
roska_rod = c("Pde6b","Nr2e3","Cnga1","Pde6a","A930006D01Rik","Rp1","Cyld","Sh2d1a","Guca1b","Faim","Gm12689","Gm12689","Gnb1","Fscn2","Reep6","Nxnl2","Nxnl1","Nrl","Gm13251","Gm13251","1700020D05Rik","Spata1","Olfr772","Slc24a1","Ybx3","Gm9958","Gm9958","Rex2","Rexo2","Ppap2c","Hmgb2")
rod_indexes = rownames(counts) %in% rod | rownames(counts) %in% roska_rod
...
the X axis. The units are the raw counts to the fourth root, to compress onto the scale.
...
pheatmap(t(sqrt(sqrt(counts[cone_indexes | rod_indexes,]))), show_colnames=TRUE, cluster_rows=TRUE, annotation=a, show_rownames=FALSE)
```
...
heatmap of the housekeeping genes, with the counts normalized to the
total number of counts for that cell. You can see the number of genes
detected correlates the overall number of reads in each cell, and that
in turn has a correlation to the normalized depth of the housekeeping
genes. The presence/absence of the housekeeping genes says more about
the variation in sequencing than the samples.

```{r variation-heatmap}
normcounts = counts / colSums(counts)
...
rownames(col_totals) = colnames(normcounts)
pheatmap(normcounts[rownames(normcounts) %in% housekeeping,], show_colnames=FALSE, cluster_rows=TRUE, show_rownames=TRUE, annotation=col_totals, drop_levels=FALSE)
```
...
```{r photoreceptor-and-housekeeping-variance, results='asis'}
photoreceptor = c(rod, roska_cone, roska_rod)
sig_varied$id = rownames(sig_varied)
```

photoreceptors:
```{r photoreceptor-variance}
...
pheatmap(normcounts[rownames(normcounts) %in% photoreceptor_varied$id,], show_colnames=FALSE, cluster_rows=TRUE, show_rownames=TRUE, annotation=col_totals, drop_levels=FALSE)
```
